<template>
    <div class="chart">
        <vue-draggable-resizable :x="451"
                                 :y="273"
                                 :w="426"
                                 :h="406"
                                 @dragging="(left, top) =>onDrag('chart1571917557000',left,top)"
                                 @resizing="(x, y, width, height) =>onResize('chart1571917557000',x, y, width, height)"
                                 @activated="onActivated('chart1571917557000')">
            <div @click="deleteChart('chart1571917557000')" class="delete">删除</div>
            <div class="chart" ref="chart1571917557000"
                               style="width: 426px;height:406px;"
                               data-width="426" data-height="406" data-x="451" data-y="273"></div>
    ...
    export default {
        data(){
            return {
                ...
                width:0,
                height:0,
                chartId:'chart1571917557000'
            }
        },
        mounted() {
            let mconfig = {"chartId":"chart1571917557000","config":{"commonConfig":{"tooltip":{"trigger":"axis","axisPointer":{"type":"shadow","label":{"show":true}}},"title":{"text":"","textStyle":{"color":"#D6BC28","fontSize":14}},"textStyle":{"color":"#fff"}},"userConfig":{"x":"TJDATE","y":[{"id":"GWYPZZMJ","name":"国务院批准总面积"},{"id":"SZFPZZMJ","name":"省政府批准总面积"}],"yAxis":[{"type":"value","name":"面积","axisLabel":{"formatter":"{value} "}}]},"dataUrl":"http://localhost:8888/api/bar/ydys/v1","width":426,"height":406,"dx":451,"dy":273},"chartType":2}
            ...
            getChartData(dataUrl).then((res)=>{
               ...
               echarts.registerTheme('chalk',jsonobj)
                this.$echarts = echarts.init(this.$refs.chart1571917557000, 'chalk', {
                    width: mconfig.config.width,
                    height: mconfig.config.height
                })
                this.$echarts.setOption(tempConfig)
                this.setPosition({id:'chart1571917557000',x:mconfig.config.dx,y:mconfig.config.dy,width:mconfig.config.width,height:mconfig.config.height})
            })
        },
        computed:{
             ...mapGetters(
