<template>
    <div class="chart">
        <vue-draggable-resizable :x="282"
                                 :y="72"
                                 :w="279"
                                 :h="224"
                                 @dragging="(left, top) =>onDrag('chart1571905831000',left,top)"
                                 @resizing="(x, y, width, height) =>onResize('chart1571905831000',x, y, width, height)"
                                 @activated="onActivated('chart1571905831000')">
            <div @click="deleteChart('chart1571905831000')" class="delete">删除</div>
            <div class="chart" ref="chart1571905831000"
                               style="width: 279px;height:224px;"
                               data-width="279" data-height="224" data-x="282" data-y="72"></div>
...
<script>
    ...
    export default {
        mounted() {
            let mconfig = {"chartId":"chart1571905831000","config":{"commonConfig":{"tooltip":{"trigger":"axis","axisPointer":{"type":"shadow","label":{"show":true}}},"title":{"text":"","textStyle":{"color":"#D6BC28","fontSize":14}},"textStyle":{"color":"#fff"}},"userConfig":{"x":"TJDATE","y":[{"id":"GWYPZZMJ","name":"国务院批准总面积"},{"id":"SZFPZZMJ","name":"省政府批准总面积"}],"yAxis":[{"type":"value","name":"面积","axisLabel":{"formatter":"{value} "}}]},"dataUrl":"http://localhost:8888/api/bar/ydys/v1","width":279,"height":224,"dx":282,"dy":72},"chartType":2}
            ...
            getChartData(dataUrl).then((res)=>{
               ...
               echarts.registerTheme('chalk',jsonobj)
                this.$echarts = echarts.init(this.$refs.chart1571905831000, 'chalk', {
                    width: mconfig.config.width,
                    height: mconfig.config.height
                })
                this.$echarts.setOption(tempConfig)
                this.setPosition('chart1571905831000',{x:mconfig.config.dx,y:mconfig.config.y,width:mconfig.config.width,height:mconfig.config.height})
            })
        },
        methods:{
            onDrag(id,x,y){
