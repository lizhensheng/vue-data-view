<template>
    ...
    <div class="leftWrapper">
        ...
        <div>
            <keep-alive>
                <lk-tabs v-model="activeName" :closable="false">
                <lk-tab-pane label="工程管理" name="first">
                    <div class="operation-wrapper">
                        <div class="operation-inner">
                            <ul>
                                <li><i class="el-icon-folder" title="新建分组" @click="addProjectFold"></i></li>
                                <li><i class="el-icon-document" title="新建页面" @click="addProjectPage"></i></li>
                            </ul>
                        </div>
                    </div>
                    
                    <lk-tree ref="treeProjects"
                             :data="treeData"
                             node-key="_id"
                             :props="defaultProps" 
                             :indent="26"
                             ...
                             :render-content="renderContent"></lk-tree>
                    <div class="split"></div>
                ...
                <lk-tab-pane label="控件" name="second">
...
<script>
    ...
    import {
        getTableNames,getDataset,getPageProjectName,getAllPageProject
        ,getDbConfigs,deleteDbConfig,hasExistPageProject,addPageProjectName
        ,editPageProjectName,getConfigByFunctionName} from 'api/dbhelper'
    export default {
        data(){
            return {
                fit:'fill',
        ...
        methods:{
                    ...
                    getDataset('',this.dbtablename,this.dbsource).then((res)=>{
                        if(res.status == 200){
                            if(res.data.data.length>0){
                                let arrNames = Object.keys(res.data.data[0])
                                let arrValues = Object.values(res.data.data[0])
            ...
            allowDrop(draggingNode, dropNode, type) {
                if(type === 'inner'){
                    return dropNode.isLeaf === false && draggingNode.isLeaf === true
                } else {
                    return true
                }
            },
            ...
            addProjectFold(){
                let currentNode = this.$refs.treeProjects.getCurrentNode()
                if(!currentNode||currentNode.pid!=='0'){
                    alert('请先选择根目录下')
                    return 
                }
                this.currentNode = currentNode
                let node = this.$refs.treeProjects.getNode(this.currentNode)
                this.$refs.treeProjects.append({pid:currentNode._id,label:'未命名文件夹',isfold:true},node)
                let childs = node.childNodes
                childs[childs.length-1].setEdit(true)
            },
            addProjectPage(){
                this.currentNode = this.$refs.treeProjects.getCurrentNode()
                let node = this.$refs.treeProjects.getNode(this.currentNode)
                this.$refs.treeProjects.append({pid:this.currentNode._id,label:'未命名文件',isfold:false},node)
                let childs = node.childNodes
                childs[childs.length-1].setEdit(true)
            },
            editBtnPlus(e,node,data) {
            ...
            appendBtn(e,node, data){
                ...
                editBtn.className = 'btn el-icon-edit'
                editBtn.onclick = (e)=>{
                    this.editBtnPlus(e,node,data)
                }
                var removeBtn = document.createElement('span')
                removeBtn.className = 'btn el-icon-delete'
                removeBtn.onclick = (e)=>{
                    this.removeBtnPlus(e,node,data)
                }
            ...
            updateFileName(e,node){
                if(node.data._id){
                    ...
                    hasExistPageProject({label:e.target.value}).then(res=>{
                        if(res.data.code === 0){
                            if(res.data.data){
                                e.target.focus()
                            }else{
                                editPageProjectName({label:e.target.value,_id:node.data._id}).then(res=>{
                                    if(res.data.code === 0){
                                        //编辑节点
                    ...
                    hasExistPageProject({label:e.target.value}).then(res=>{
                        if(res.data.code === 0){
                            if(res.data.data){
                                ...
                                addPageProjectName({label:e.target.value,pid:currentNode._id,isfold:node.data.isfold}).then(res=>{
                                    if(res.data.code === 0){
                                        //删除临时节点
                                        let _node = this.$refs.treeProjects.getNode(currentNode)
                                        let _childs = _node.childNodes

                                        _childs[_childs.length-1].remove()
                                        //新增节点
                                            this.$refs.treeProjects.append({pid:currentNode._id,label:e.target.value,isfold:node.data.isfold},_node)
                                    }else{
            ...
            renderContent(h, { node, data, store }) {
                if(node.isEdit === false || node.level === 1){
                    return (
                    <div class="custom-tree-node" onmouseenter={(e)=>this.appendBtn(e,node, data)} onmouseleave={(e)=>this.removeBtn(e)}>
                        <span>{node.label}</span>
                    </div>);
                }
                else{
                    return (<div class="custom-tree-node"><input type='text' value={node.label} onblur={(e)=>{this.updateFileName(e,node)}}  autofocus="autofocus" /></div>)
                }
            },
            ...mapMutations({
...
<!-- <lk-tab-pane label="数据源" name="third">
                    <el-collapse v-model="activeControls">
